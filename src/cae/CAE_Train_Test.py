...
def vis_output_for_edge_cloud_input(model_encoder, model_decoder, criterion, vis_dataset, save_fig_path,
                                    device, epoch=None, loss_gate=None, epoch_gate=None, dimension=2):
    model_encoder.eval()
    model_decoder.eval()
        ...
        for x_cloud in vis_dataloader:
            ...
            for i in range(vis_batch_size):
                ...
                if loss_gate is not None and epoch > epoch_gate and vis_loss_ > loss_gate:
                    ...
                    plot_env_scatter(save_fig_path=save_large_loss_fig_path_i, env_cloud=decoder_out_[i], dimension=dimension)

                # save output fig
                plot_env_scatter(save_fig_path=save_fig_path_i, env_cloud=decoder_out_[i], dimension=dimension)

                # save origin fig
                if not origin_save_flag[vis_fig_cnt]:
                    save_fig_path_i_origin = save_fig_path + "fig_cnt" + str(vis_fig_cnt) + '_origin'
                    plot_env_scatter(save_fig_path=save_fig_path_i_origin, env_cloud=x_cloud_[i], dimension=dimension)
...
def Train_Eval_CAE_main_C3D():
    ...
    # train_data_load_file = "../../../output/data/envs_for_global_lidar_info/cloud_edge_1400_train_MPN_test.npy"
    # train_data_load_file = "../../../output/data/S2D/obs_cloud_30000.npy"
    train_data_load_file = "../../data/train/c3d/c3d_obs_cloud_40000_train.npy"
    test_data_load_file = "../../data/train/c3d/c3d_obs_cloud_10000_test.npy"
